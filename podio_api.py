...
def create_tables(podio):
    ...
    if type(workspaces) is list:
        # Acessando o BD
        mydb = psycopg2.connect(host=os.environ['POSTGRES_HOST'], user=os.environ['POSTGRES_USERNAME'], password=os.environ['POSTGRES_PASSWORD'], dbname=os.environ['POSTGRES_DATABASE'], port=os.environ['POSTGRES_PORT'])
