                ...
                try:
                    for step in range(0, number_of_items, 500):
                        ...
                        for item in items:
                            ...
                            cursor.execute(f"SELECT `last_event_on` FROM {table_name} WHERE `id`='{item['item_id']}'")
                            ...
                            if cursor.rowcount == 0 or last_event_on_podio > last_event_on_db:
                                ...
                                try:
                                    ...
                                    hour = get_hour()
                                    message = f'Aplicativo alterado. Detalhes: {err}. Excluindo a tabela "{table_name}"...'
                                    logger.info(message)
                                    # send_to_bot(f'{hour} -> {message}')
                                    cursor.execute(f"DROP TABLE podio.{table_name}")
                                    raise dbError('Tabela excluída!') from err
